...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, anims):
        ...
        # Create a smaller collision rect than the visual sprite
        # Shrink width by 40% so player must jump 1-tile gaps
        collision_width = int(TILE_SIZE * 0.6)
        collision_height = TILE_SIZE
        self.rect = pygame.Rect(pos[0], pos[1], collision_width, collision_height)

        # Center the collision box horizontally within where the sprite would be
        sprite_center_x = pos[0] + TILE_SIZE // 2
        self.rect.centerx = sprite_center_x
